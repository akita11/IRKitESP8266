/*
 Copyright (C) 2018 Junichi Akita (@akita11, akita@ifdl.jp)

 This program is based on Toshiyuki Kawashima's ESP8266IRKit
...
#include <ir_Trotec.h>
#include <Adafruit_NeoPixel.h> // for IRkitESP8266 by akita11
...
const int led_pin = 12; // for IRkitESP8266 by akita11
const int sw_pin = 13;  // for IRkitESP8266 by akita11
...
void setup() {
  ...
  if (gSetting.init != DONE && gSetting.init != WAIT_RESPONSE) {
    ...
    WiFi.mode(WIFI_STA);
    Serial.print("conecting to SSID:"); Serial.println(ssid);
...
void loop() {
  ...
  if (gSetting.init == WAIT_RESPONSE) {
    ...
    if (irrecv.decode(&results)) {
      // status: blue blink // for IRkitESP8266 by akita11
      //dump(&results);
...
//      body = base64::encode(sharedbuffer, irpacker_safelength(&packer_state));
      body = base64::encode((uint8_t *)sharedbuffer, irpacker_safelength(&packer_state));
